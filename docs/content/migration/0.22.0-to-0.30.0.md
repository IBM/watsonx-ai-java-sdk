...
If you register interceptors as inline lambdas on the service builder, the type argument is inferred from the setter, so **no change is required**.
...
This affects you only if you provide a custom REST client through the [REST Client SPI](/advanced/spi). The `chatStreaming()` method of `ChatRestClient`, `DeploymentRestClient`, and `ModelGatewayRestClient` now receives a `ChatClientContext` parameterized with the service's request type, and an override must match it:
...
This release adds `ModelGatewayService` for sending chat requests to third-party models through the IBM watsonx.ai Model Gateway endpoint. It is a new, additive API with no migration required. See the [Model Gateway](/services/model-gateway) documentation for details.
...
This release adds `ModelGatewayCatalogService` for listing and fetching models configured in the IBM watsonx.ai Model Gateway. It is a new, additive API with no migration required. See the [Model Gateway Catalog](/services/model-gateway/catalog) documentation for details.
