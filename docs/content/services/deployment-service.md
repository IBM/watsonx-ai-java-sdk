...
The `DeploymentService` allows you to interact with **models deployed in IBM watsonx.ai deployment spaces**. Instead of referencing a `modelId`, every request targets a `deploymentId` (the identifier of an already-deployed asset). It supports the same operations as [`ChatService`](/services/chat-service) and [`TimeSeriesService`](/services/time-series-service) (chat, streaming chat, time series forecasting), plus the ability to inspect a deployment's metadata via `findById`.
...
- [ChatService Documentation](/services/chat-service)
- [Time Series Service Documentation](/services/time-series-service)
