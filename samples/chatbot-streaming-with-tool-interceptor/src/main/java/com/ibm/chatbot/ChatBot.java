...
public class ChatBot {
     ...
     * @return a CompletableFuture that completes with the final AssistantMessage
     *         containing the assistant's response. The future may complete
    ...
    public CompletableFuture<AssistantMessage> chat(String message) {
        ...
        chatService.chatStreaming(memory.getMemory(), tools, new ChatHandler() {
            ...
            public void onCompleteResponse(ChatResponse completeResponse) {
                ...
                if (!assistantMessage.hasToolCalls()) {
                    ...
                    var toolMessages = assistantMessage.processTools((toolName, toolArgs) -> {
                        return switch(toolName) {
                            ...
                            case "get_current_time" -> {
                                ...
                                logger.info("Executed get_current_time function with result: " + result);
                                yield LocalDateTime.now();
                            }
                            default -> throw new IllegalArgumentException("Unexpected value: " + toolName);
                        };
                    });
                    ...
                    try {
                        assistantMessage = chatService.chatStreaming(memory.getMemory(), tools, this).get().toAssistantMessage();
                        future.complete(assistantMessage);
                    } catch (Exception e) {
